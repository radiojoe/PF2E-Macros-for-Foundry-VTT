/*
 This Macro will roll an athletics check to Trip including an item bonus equal to the highest potency rune
 bonus that you have equipped on a weapon with the Trip trait.
...
for (var i = 0; i < weapons.length; i++) {
    if (weapons[i].data.equipped) {
        if (weapons[i].data.traits.value.indexOf('trip') != -1) {
...
if (highest_item_bonus_weapon) {
    ...
    ui.notifications.warn("You must have a weapon with the Trip trait equipped.");
}
